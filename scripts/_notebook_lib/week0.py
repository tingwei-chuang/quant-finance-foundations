...
def week(solution: bool) -> list[nbf.NotebookNode]:
    ...
    cells += [
        ...
        *quiz_cells(
            ...
            items=[
                (
                    "價格從 100 漲到 110，simple return 與 log return 分別約是多少？",
                    ["10% 與 9.53%", "10% 與 10%", "9.53% 與 10%", "11% 與 10.5%"],
                    "A",
                    "simple = 110/100 − 1 = 10%；log = ln(1.1) ≈ 9.53%。漲幅越大兩者差越大。",
                ),
                (
                    "本專案所有 notebook 預設使用合成資料的主要原因是？",
                    [
                        "真實資料不夠準確",
                        "可重現、可離線執行、且避免資料授權問題",
                        "合成資料報酬較高",
                    ...
                    "B",
                    "合成資料讓所有結果完全可重現、不需網路，也不涉及第三方資料的使用條款。",
                ),
                (
                    "「樣本平均是隨機變數」的意思是？",
                    [
                        "它每次計算結果都不同",
                        "它依賴抽到的樣本，換一組樣本就會得到不同的值",
                        "它無法計算",
                        "它一定不準",
                    ],
                    "B",
                    "樣本平均是資料的函數；資料是隨機抽樣的結果，所以樣本平均本身有抽樣分布。",
                ),
                (
                    "對時間序列做隨機 train/test 切分的主要問題是？",
                    [
                        "計算太慢",
                        "訓練集可能包含「未來」資訊，造成 look-ahead bias",
                        "測試集太小",
                        "無法畫圖",
                    ],
                    "B",
                    "隨機切分讓模型用未來資料學習、再「預測」過去，結果系統性過度樂觀。",
                ),
            ],
        ),
        mistakes(
